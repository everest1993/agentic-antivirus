#!/usr/bin/python
# Vendored locally from elastic/ember (features.py) to avoid importing the full
# `ember` package at runtime for feature extraction.
...
class PEFeatureExtractor(object):
    ...
    def raw_features(self, bytez):
        # LIEF >= 0.17 no longer exposes legacy exception symbols at module level.
        # Build a compatible exception tuple dynamically for old/new versions.
        legacy_error_names = (
            "bad_format",
            "bad_file",
            "pe_error",
        ...
        lief_errors = tuple(
            err
            for name in legacy_error_names
            for err in [getattr(lief, name, None)]
